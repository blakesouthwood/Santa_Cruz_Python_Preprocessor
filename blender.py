...
#ifmacros=['starter'] #initialized it
...
##############################################################################
##   may 17th 2021 monday version convert_case_numbers_to_strings():
##############################################################################
...
# MODIFIED ON MAY 17TH TO DETECT IF MACROS BEING USED OR NOT

#but later I can make a new method for word constants which will easily be converted to strings
#the way they look in C  with this look case RUNNING:   case WINTER:
...
#################################################################
	...
	#macro_found[0] == "False": #this reset list flag macro_found to False by default
#end def
...
#	case_numbers_to_strings() 
	 ...
	 #macros=['starter']  #may 17th coding will hold either True or False
...
def starter_sequence_mode_2():
	print("starter_sequence_mode_2 called")
	...
	if valve[0] == True: #meaning numbers in cases detected
		...
		print("===========--------==========")
		print(switchcasetester)
		print("input state of the switch case =======")
		#this is the only line I need to put into faucet_valve() in march_firefall_yosemite_falls
		print("about to test if macros in this numbers switch string")
		#just realized for my flag I need to use a list to ensure it works
		#detect_if_thru_and_or_to_macros_exist()
		##### CONVERT CASE NUMBERS TO STRINGS()  
		print("triggering convert_case_numbers_to_strings()")
		convert_case_numbers_to_strings()
		...
		print("right here look at the switch see if it's totally unfurled")
		print(switchcasetester) #this is what it will feed into the parser
...
def change_to_into_thru():
	global switchcasetester
	print("just called change_to_into_thru() method")
	...
	for line in switchcasetester.splitlines():
		...
		if "case" and "to" in line:  #this means the macro to
			...
			pass
	print("the result of swapping to with thru")
	print(switchcasetester)
...
def check_if_first_case_is_number():
	...
	for line in switchcasetester.splitlines():
		if "case" in line:  #error this doesn't account for using thru or to
			...
			if sum(map(str.isdigit, line)) == 2 and "thru" or "to" in line and "fallthru" not in line:
...
# first I need to have it check if "thru" or "to" in switchcasetester
# be sure to do that
...
def faucet_valve():
	...
	#print(valve[0])
	#print("firstcase =",firstcase)
	if firstcase == True: #this meant in early testing that if true it's a number 
		#print("here means firstcase == True meaning a NUMBER:")
		convert_case_numbers_to_strings()
	else:
		pass
...
def flow_valve_input(y):  #this determines if switch case string is numbers or words
    valve[0]='starter' #default settings 
    valve[1]='starter' #default settings
    #This tests the entire switch string    
    
    print("Flow Valve input")
    #this code only checks the first case if it's a number after case
    #
    ...
    valve[0] = check_if_number_in_string(toocool)            #looks in case line returns True or False
    valve[1] = toocool                                       #put case name/number into valve[1]
    print("output from FLOW VALVE=",valve[0],"and",valve[1])
    if "to" or "thru" in valve[1]:   #however it's only looking in the first case where there might not be a macro ... aha
        print("valve[1]=",valve[1])
        print("macros in use in first case")
       # print("YES VICTORY MACROS IN USE")
     #maybe we don't need to know if macros are used at this juncture afterall 
     #but at least we know that it's a numbers case and that's all that's important here.   
...
def endswitch(y): #pulls in sw 
	...
	##################### True means cases are numbers False means words in stings
	#valve[0] = False #this is hardcoded False for this part
	print("valve[0] =", valve[0])
	if valve[0] == True: #this says numbers = True
		print("......DETECTED NUMBER SWITCH........")
		print("valve[0] == True thus numbers", valve[0])
		print("valve[1]=",valve[1])
		########################################
		#I need to put the detect numbers here  
		if "thru" or "to" in valve[1]:
			print("Tahoe here we come in a GTI")
		else:
			print("no thru or not in first case")
		#########################################
		flush_lists() #new just addd april 7th triggered before parser starts
		parser_mode_2(y) #this is numbers parsing
...
'''
good one count = 6
let's look inside of case_main_body_list_with_tail
['starter',
'\t\tprint("where\'s the cream house!")\n\t\tprint("tastes good")\n\t\tbreak',
"\t\tprint('fallen leaf lake')\n\t\tfallthru('driving car')",
...
'\t\tprint(\'fast laptop\')\n\t\tprint("when will the code work...")\n\t\t',
'\t\tprint(\'six walking duck de fa ul t\')\n\t\tprint("flying geese")']
the length of case_main_body_list_with_tail = 5
'''
