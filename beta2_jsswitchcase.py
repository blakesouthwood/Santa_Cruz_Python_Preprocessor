#dec 15th 8 pm pacific time in Silicon Vally.
...
In addition to C style comments for multiline and single // line comments I figured out using braces and bypassing indentation
though the output is indented for the python to actually work but you don't have to think about indentation in your functions.
...
them later. What a rush to finally see it actaully work.
...
def stars():
	...
	for item in listnow:
		...
		if  item.startswith("f") == True:
			#print('fallthru found')
			cru0.append("fallthru('" + str(counter) + "')")
			counter += 1
...
def switch_code_gen():
	...
	print("now printing out the taillist")
	print(cranberries)
