...
##### flush_lists() ##############  working on this on April 12th, 2021
def flush_lists():
	...
	print("varholder=[]",varholder) #this also needs to start as a virgin
	...
	for line in switchcasetester.splitlines():   #so if they are together  stacked
		...
		if mycounter in line_numbers_of_first_cases:
			...
			if "default" in line:
				...
				#new exception covers if break in line
			elif "break" in line:
				second_word = line.split()[0] #gets the first word which is break
			#else:
...
#numbers as strings after they are converted into strings and macros expanded 
...
def reset():  #this now does nothing.
	...
	del sw
	sw ='' #this empties it
	switchcasetester=None
	del switchcasetester
	emptyvar=''
	switchcasetester=emptyvar #this empties it
	return sw;
...
def get_location_of_case(listname,word):
    print("GET LOCATION OF CASE function called ")
    print("##############################")
    print("##############################")
    print("##############################")
    print("##############################")
    ...
    print("listname=",listname,"word",word)
    print("=======================================")
    print('what is suddenly wrong this simple method?')
    #print("this is searching for ",word)  #below took out  + "'" + both sides of word
    #does this actually work 
    print("detect if a word is in a list it's true or false")


    #### explaination: this is looking for the locatin
    #### of the word in the list in simple terms

    #if word in palmtrees:
    answer =eval("" + listname + ".index("  + word + ")")
    

    print('palmtrees',palmtrees)
    print("answer position in the list=",answer)
...
#######################################
	...
	for line in switchcasetester.splitlines():
		beta = mycounter-1
		if "case" in line:
			#the line is a string convert it to lowercase
			#cleverline = line.lowercase() # brilliant it's now lowercase ha ha ha
			smart=line.split() #separates case from casename
			#print(smart[1])
			#bright = smart.lowercase()
...
def stage_one():  #this calls start_trigger()
    ...
    starbuckslist=[]
    digitalcandy=[]
    start_trigger() # this is the main starting match of code
...
##################################################
	mytrace('magictimenow')
	print("=== magictimenow()======= testing for bug in line_numbers_of_first_cases  list == ")
	...
	coolcounter=0
	#print("about to test loop looking for first case")
	#this looks for case in this line but no case in line above it
	for item in ifcaseinline:
		if item == "true" and ifcaseinline[coolcounter-1] == "false":
...
##############  I should make this a separate function ###################################
	...
	#sure that "case" is not in the previous line determined by the word 'false'
	#the trontime list only has trues and falses so true if case in line otherwise false
	print("this is in magictimenow() =========== debugging april 15th, 2021 6:46am==")
	print("===== testing what is in this switchcasetester which I am")
	print("===== deriviing filling the line_numbers_of_first_cases  list")
	print("===== and I need see exactly what it is accessing in the switch string")
	print("data on line_numbers_of_first_cases ")
	print("the length of it is ", len(line_numbers_of_first_cases))
	
	if len(line_numbers_of_first_cases) >0:
		line_numbers_of_first_cases.clear()
		print("just cleared list line_numbers_of_first_cases")
	else:
		print("the list line_numbers_of_first_cases was already empty")
		pass
	
	print("let's see what's in it ",line_numbers_of_first_cases)
	print("======")
	print("switchcasetester=",switchcasetester)
	mytrace(switchcasetester)
	...
	for line in switchcasetester.splitlines():
		...
		if "case" in line and "case" not in trontime[acounter-1] :  #tests if no case above this line with case
			...
			acounter +=1
			continue
 ### all this loop below does is put the line number of default into 
 ### line_numbers_of_first_cases
 ### and appends it ot firstcaselist
...
#counts tabs
#### this is counting tabs which I decided to nix and not use
#### =================== truckeeriver() ==================
def truckeeriver():
	mytrace('truckeeriver')
	print("This is truckeeriver inside of stage_three for words strings")
	casecounter=0
	mycounter = 1
...
#april 15th, 2021
#here we have palmtrees and line_numbers_of_first_cases
#manipulated and palmtrees filled in rule_the_earth()
...
###################################
	...
	plumtree=''
	total_word_count=''
	############ this is new april 15th clearing it out 
	if len(palmtrees) > 0:
		palmtrees.clear()
	...
	##for case sets I just go thru till case not in line and fill a new list
	print("switchcasetester=",switchcasetester)
	print("appletree[0]",appletree[0])
	print('this is in rule_the_earth()')
	#switchasetester
	for line in appletree[0].splitlines():   #so if they are together  stacked
		#batterondeck represents previous loop count
		#go thru case list here as the feeder
		if mycounter in line_numbers_of_first_cases:
			#print(line)
...
# I need to add my code adjustment here 
	...
	## if last two of palmtrees = -1 default and -2 starter then take them off
	print("palmtrees=",palmtrees)
	###==== thjis is new to fix bug april 15th 
	if palmtrees[-1] == "default" and palmtrees[-2] == "starter":
		palmtrees.pop() #deletes last item from palmtrees
		palmtrees.pop() #de;etes ;ast item from palmtreees
	#### === end of bug fix 
	print("palmtrees=",palmtrees)
	starbuckslist=[] #resets it
	#filling starbuckslist
	#####################################
	if len(starbuckslist) >0:
		startbucklist.clear()
...
###################################
    ...
    print("convert to twin list() called")
...
def change_to_string(x,y):
	...
	switchcasetester=''; switchcasetester=None; switchcasetester = peach
...
def stage_four():
	#print(switchcasetester) 
	print("STAGE FOUR Called which calls the bug is in here somewhere")
	print("magictimenow()")
	print('convert_to_twin_list()')
	print("special_addition_to_digital_candy()")
	print("rule_the_earth()")

	print("=== STAGE FOUR == testingwhat it sees in stage four() ======")
	mytrace('stage_four()')
	#print('stage four')
	...
	print("==============")
	print("appletree[0]=",appletree[0])
	#this puts the contenst of switchcase tester into apppletree[0]
	appletree[0] = switchcasetester
	print("appletree[0] now =",appletree[0])
...
snowman.append('starter')
 #I was clearing it out here good thing I noticed it
casecaptured=''
mrcase=''
def case_tail_list_maker(x,y):  #two vars x and y are case locations from digitalcandy
   ...
   ##############################################
    ...
    ##############################################
    ##############################################
    print("at this point line 7762 what is in switchcasetester")
    print(switchcasetester)
...
def crashing_waves_at_beach():
	...
	################################################
	print('line 7965')
	print("llok below here ")
	print('what does roadrunner have at this point',roadrunner)
	print("what is in here ?????=====")
	print("looking at wilecoyote", wilecoyote)

	print("I might need to add if case in line also")
	## this gets the location of the case name in wilecoyote inside of palmtrees list
	###
	
	print("=======  ANTACRTICA DAM IT ====")
	
	
	i = 0
	while i < len(palmtrees):
		print(palmtrees[i])
		#this way I would know the location
		i = i + 1

	print("more experimentaitoning =====")
	aword = 'laugh'    
	string = 'This is laughing laugh'.split(" ")
	daword = string.index(aword)
	print("DAWORD get number",daword)
	...
	#this says get the location of item in wilecoyote in list palmtrees to get location
		#this is looking in the list palmtrees.
		#print("the item is",item)
		#print("===ufos in alaska=====major testing here ======")
		#if item in palmtrees:
		#	print("yes in palmtrees")
		#else:
		#	print("no not in palmtrees")
		#this is to get the location of a word in list palmtrees
...
def stage_six():
    ...
    if firstcase == True:  #which means numbers like case 4:
        f22_fighter_jet() #this was before I decided to splitit into two modes
    else:
        p51_mustang()
...
def switch_code_gen():
	...
	#this has the break/fallthru tacked on beneath it so instead 
	#of the hamburer it has bottom bun of the break/fallthru attached to it
...
#this is the input for exp for switch(exp)
	##############
	#this is the input that is fed into THIS particular switch case
	#when it's run in python which is really a while True loop with if elif else
	topvars2 = "\nexp = varholder[0]\n\n"
	...
	#this makes the python caselists at the top of the switch
	#which are referenced by python
	#it builds an individual caselist for a case section
	#and appends it to testlist
	...
	#this works this creates the strings of the caselist at the top above switch correctly
###============================================
###### This is where the caselists at the top of a python switch case
###### are dynamically
# caselists concatted to string cool
	cool=''
...
#===case sections three parts at once ==")
 ...
 #== LOOP
	...
	print("")
	#this is where the python switch case if elif else is run
	exec(sweet)  #this is the building of the string of python code strings
	...
	zapo=''
	sweet = zapo
	#this clears out what was in sweet
...
def flush_lists_previous():
	...
	gti=[]
	royallist=[]
	#just added theseto see if it would help
	einstein=[]
	palmtrees=[]
	cranberries=[]
	starbuckslist=[]
...
def starter_sequence_mode_1():
	...
	if valve[0] == False: #means cases are words like "apple pie"
		...
		#wilecoyote=[]
		#print("wilecoyote=",wilecoyote)
		#print("here I am deleting switchcaseter")
		############ april 12th zapping out switchasetester
		global y
		y =None
		y=''
		global switchcasetester
		...
		sw=''
		print("sw=",sw)
		print("====== empty switchcasetester now =====")
		print("after code generated empty switchcasetester")
		# this should reset switchcasetester to nothing
		#global switchcasetester
		print("switchcasetester=",switchcasetester)
		switchcasetester=None
		plum =''
		switchcasetester = plum
		print("now what is in switchcasetester",switchcasetester)
		print('lenght of switchcaseter=',len(switchcasetester))
		smartcasemanager=[]
		print("smartcasemanager=",smartcasemanager)
		british=[]
		print("british=",british)
...
##################################
'''
# get first case in switch case string
# get first case line
# remove tabs from the case line
# test if number in first case line yes = True no = False
# looks in case line
# put case name/number into valve[1]
# empties lists before proceeding with parser and codegen
'''
def flow_valve_input(y):  #this determines if switch case string is numbers or words
    ...
    valve[0] = check_if_number_in_string(toocool)            
    valve[1] = toocool                                       
    print("valve[0]=",valve[0]," and valve[1]=",valve[1])
    flush_lists() #empties the lists acting as reset
    		      #purpose served is if multiple switches used the previous 
    		      #list data in the parser and code gen are wiped clean
################################################
...
# this looks quickly at the first case in the switch string
	...
	# if it's a number it returns True else it returns False
	##################################################
...
def endswitch(y): #pulls in sw 
	#added April 9th 2021
	#added Tuesday April 13th
	appletree.append(y)
	print("appletree[0]=",appletree[0])
	print("if the switch cases are together here I will have to eliminate the top one leaving")
	print("just the bottom switch string")
	...
	#appletree[0]='empty'
	#print("now appletree[0]=",appletree[0])
	
	onion =''
	switchcasetester = onion
	mytrace("endswitch() in switch_cat called")
	print("we have an input argument parameter y",y)
	#there are 2 parsers and 2 code generators
	# parser_mode_1 does words as strings
	# parser_mode_2 does numbers as strings with macros
	#####################
	flow_valve_input(y)   #puts True or False into valve[0] added April 2nd, 2021
	#####################
	# if numbers in case = True
	if valve[0] == True: 
		parser_mode_2(y) 
	else: 
	#if numbers in case == False    
		parser_mode_1(y) 
